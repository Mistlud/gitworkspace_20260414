...
| 탭 | 내용 |
|---|---|
| 홈 | 앱 설명 / Provider별 등록 방법 / 번역·교정 모드 안내 / 이력·비교·설정 등 기타 기능 안내 / 선택된 API 상태 및 인증 안내 |
| 번역 | 모드 토글 + 도착어 선택 + API 상태 인디케이터 + 입력 글자 수·예상 토큰 수 + 텍스트 입력/결과 화면 + 결과 비교 |
| 이력 | 번역·교정 이력 목록 (최대 10건, 앱 종료 시 초기화) + 입력/결과 뷰 + 결과 복사 + 다시 편집 |
| 비교 | 두 텍스트의 로컬 비교 + 차이 강조 + 글자 수·예상 토큰 수 + 차이 탐색 |
