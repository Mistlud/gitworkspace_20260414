...
### 버그 수정 — promptsFilePath 재귀 호출 (2026-04-21)
- `main.js`의 `promptsFilePath`가 개발 환경(`app.isPackaged === false`)에서 자기 자신을 재귀 호출하는 버그 수정
- `promptsFilePath()` → `path.join(__dirname, 'prompts.json')`으로 교체

### 뒤집기 버튼 (2026-04-21)
- 번역 탭 두 패널 사이에 `⇐` 버튼 추가
- 결과창 텍스트를 입력창으로 이동 후 결과창 초기화 (복사 버튼 숨김)
- 결과창이 비어 있을 때는 동작하지 않음

### UI 개선 (2026-04-21)
- 뒤집기 버튼 디자인 개선: `.flip-col` 컬럼 제거, 버튼을 1px 구분선 위에 `position: absolute`로 부유
- 번역 탭 입력창·결과창 스크롤바 스타일 추가 (4px, 다크 테마) — 다른 탭과 통일
- 홈 탭 설명 전면 재작성:
  - 사용 방법 4단계로 확장 (키 1회 등록, 뒤집기 버튼 안내 포함)
  - 교정 모드에 언어 자동 감지 특징 명시
  - 기타 기능에 이력 되돌리기, 버튼 위치·동작 상세 설명 추가
  - 아이콘 오표기 수정 (⊤→📌, ◑→🔆)
- 홈 탭 레이아웃 재설계:
  - 헤더(`LLMTransTool`) 고정, 아래 섹션만 스크롤
  - 스크롤바를 화면 우측 끝에 배치
  - 콘텐츠 폭 660px 중앙 정렬, 섹션 카드 스타일 적용
  - 헤더 박스 제거 (배경·테두리 없이 텍스트만)
